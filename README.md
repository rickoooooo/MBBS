...
## Interfaces
An interface allows the BBS to communicate over a given connection. Currently there are two interfaces written: `CommInterfaceMeshtasticTCP` and `CommInterfaceTCP`. All interface objects inherit from the base `CommInterface` class. The Meshtastic TCP interface allows the BBS to communicate with a Meshtastic radio via TCP. The TCP interface allows users to connect to the BBS via TCP instead of using Meshtastic. This can be useful for debugging or if you just want users to also be able to connect via TCP/IP.
